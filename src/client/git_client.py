...
class GitClient:
    ...
    def _create_authenticated_url(self):
        ...
        """
        logging.info("Creating authenticated URL")

        username_secret_id = os.environ.get("GIT_USERNAME_ID")
        token_secret_id = os.environ.get("GIT_TOKEN_ID")

        secrets_man_client = SecretsManagerClient()

        username = secrets_man_client.get_secret(username_secret_id)
        token = secrets_man_client.get_secret(token_secret_id)
